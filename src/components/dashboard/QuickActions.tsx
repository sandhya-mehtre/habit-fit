...
const QuickActions = ({ onAddHabit, onAddWorkout, onAddWeight, onAddSleep }: QuickActionsProps) => {
  ...
  const actions = [
    { label: 'New Habit',   icon: '✅', color: 'from-emerald-500 to-emerald-600', onClick: onAddHabit },
    { label: 'Log Workout', icon: '💪', color: 'from-primary-500 to-primary-600', onClick: onAddWorkout },
    { label: 'Log Weight',  icon: '⚖️', color: 'from-amber-500 to-amber-600',    onClick: onAddWeight },
    { label: 'Log Sleep',   icon: '😴', color: 'from-violet-500 to-violet-600',  onClick: onAddSleep },
    { label: 'View Analytics', icon: '📊', color: 'from-pink-500 to-pink-600',  onClick: () => navigate('/analytics') },
  ];
    ...
    <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {actions.map((action, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${action.color} flex items-center justify-center text-white text-lg`}>
              {action.icon}
            </div>
